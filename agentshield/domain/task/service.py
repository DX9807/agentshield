...
from .models import Task, TaskCapability, TaskStatus
...
class TaskService:
    ...
    async def create_task(
        ...
        expires_at = now + timedelta(minutes=task_data.expires_in_minutes)
    ...
    async def update_task(
        ...
        if update_data.expires_in_minutes is not None:
            now = datetime.now(UTC)
            task.expires_at = now + timedelta(minutes=update_data.expires_in_minutes)
        ...
        query = (
            ...
            .where(
                and_(
                    ...
                    TaskCapability.is_active.is_(True),
                )
            )
        )
    ...
    async def get_task_context(self, task_id: UUID) -> dict[str, Any] | None:
