...
class Task(BaseModel, AuditMixin):
    ...
    capabilities = relationship(
        "TaskCapability", back_populates="task", cascade="all, delete-orphan"
    )
    # events = relationship("SecurityEvent", back_populates="task")  # Reserved for future phase
    ...
    def is_active(self) -> bool:
        ...
        return self.status == TaskStatus.ACTIVE and exp > now
    ...
    def extend(self, additional_minutes: int) -> None:
        ...
        if self.is_active:
            now = datetime.now(UTC)
            current_exp = self.expires_at or now
            if current_exp.tzinfo is None:
                current_exp = current_exp.replace(tzinfo=UTC)
            base_time = max(now, current_exp)
            self.expires_at = base_time + timedelta(minutes=additional_minutes)
...
class TaskCapability(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint("task_id", "capability_id", name="uq_task_capability"),
