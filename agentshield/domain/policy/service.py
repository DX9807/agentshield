...
from ...schemas.policy import PolicyCreate, PolicyUpdate, PolicyCondition
...
class PolicyService:
    ...
    async def create_policy(
        ...
        policy = Policy(
            ...
            conditions=[cond.model_dump() for cond in policy_data.conditions] if policy_data.conditions else None,
    ...
    async def list_policies(
        ...
        # Pagination
        offset = (page - 1) * limit
        query = query.order_by(Policy.priority.asc(), Policy.order.asc()).offset(offset).limit(limit)
    ...
    async def delete_policy(self, policy_id: UUID) -> bool:
        ...
        logger.info(
            f"Policy deleted: {policy.name} ({policy.id})",
    ...
    async def enable_policy(self, policy_id: UUID) -> Policy:
        ...
        logger.info(
            f"Policy enabled: {policy.name} ({policy.id})",
            extra={"policy_id": str(policy_id)}
        )
    ...
    async def disable_policy(self, policy_id: UUID) -> Policy:
        ...
        logger.info(
            f"Policy disabled: {policy.name} ({policy.id})",
            extra={"policy_id": str(policy_id)}
        )
    ...
    async def _clear_policy_cache(self, policy: Policy) -> None:
        ...
        await redis_client.delete(
            f"policy:{policy.id}",
            f"policies:agent:{policy.agent_id}:action:{policy.action}" if policy.agent_id else None,
        )
