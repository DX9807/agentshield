...
class PolicyEngine:
            ...
            .where(
                and_(
                    ...
                    or_(
                        ...
                        Policy.agent_id.is_(None),  # Global policies
                    ),
    ...
    async def _calculate_risk_score(
        ...
        if request.resource and (
            "admin" in request.resource.lower()
            or "iam" in request.resource.lower()
            or "secret" in request.resource.lower()
        ):
            base_score += 15
    ...
    async def get_policy_statistics(self) -> dict[str, Any]:
        ...
        enabled_query = select(func.count()).where(Policy.enabled.is_(True))
