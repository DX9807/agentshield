...
class AgentService:
    ...
    async def register_agent(
        ...
        agent = Agent(
            ...
            status=AgentStatus.PENDING_APPROVAL if agent_data.risk_level in [
    ...
    async def authenticate_agent(self, agent_id: UUID, api_key: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Agent authenticated: {agent.name} ({agent.id})",
    ...
    async def delete_agent(self, agent_id: UUID) -> bool:
        ...
        logger.info(
            f"Agent deactivated: {agent.name} ({agent.id})",
            extra={"agent_id": str(agent_id)}
        )
            ...
            .where(
                and_(
                    ...
                    or_(
                        ...
                        AgentCapability.expires_at > datetime.utcnow(),
                    ),
                )
            )
        )
    ...
    async def revoke_capability(
        ...
        logger.info(
            f"Capability revoked from agent",
            extra={
                "agent_id": str(agent_id),
                "capability_id": str(capability_id),
            }
    ...
    async def _get_capability_by_id(self, capability_id: UUID) -> Optional[Capability]:
        """Get capability by ID."""
        query = select(Capability).where(Capability.id == capability_id)
        result = await self.session.execute(query)
        return result.scalar_one_or_none()
    
    async def _get_active_credential(self, agent_id: UUID) -> Optional[AgentCredential]:
            ...
            .where(
                and_(
                    ...
                    or_(
                        ...
                        AgentCredential.expires_at > datetime.utcnow(),
                    ),
                )
            )
